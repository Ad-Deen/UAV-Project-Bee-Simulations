...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        # print(f"min = {print(depth.min())}, max ={print(depth.max())}")
        
        # Compute 3D points (downsampled for performance)
        small_size = (300, 300)  # Downsample to 100x100 for real-time
        depth_small = cv2.resize(depth, small_size, interpolation=cv2.INTER_NEAREST)
        depth_small= np.round(1.0 / (depth_small + 1e-6), 4)
        # Prepare meshgrid for pixel coordinates
        ...
        # Scale intrinsics for small resolution
        f_x_small = f_x * scale_x
        f_y_small = f_y * scale_y
        c_x_small = c_x * scale_x
        c_y_small = c_y * scale_y

        # Compute 3D points using properly scaled intrinsics
        X = (u_small - c_x_small) * depth_small / f_x_small
        Y = -(v_small - c_y_small) * depth_small / f_y_small  # Flip Y-axis for right-handed coords
        Z = -depth_small
