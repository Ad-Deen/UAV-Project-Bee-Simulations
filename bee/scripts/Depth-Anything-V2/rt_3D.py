...
import numpy as np
import torch
import open3d as o3d  # Added for point cloud visualization
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        Y = (v_small - c_y * scale_y) * depth_small / (f_y * scale_y)
        Z = depth_small
    ...
    """
    # Alternative: Matplotlib visualization (slower, uncomment to use)
    ...
    while cap.isOpened():
        ...
        # Downsampled 3D points
        depth_small = cv2.resize(depth, (100, 100))
        u_small, v_small = np.meshgrid(np.arange(100), np.arange(100))
        scale_x = 100 / 640
        scale_y = 100 / 480
        X = (u_small - c_x * scale_x) * depth_small / (f_x * scale_x)
        Y = (v_small - c_y * scale_y) * depth_small / (f_y * scale_y)
