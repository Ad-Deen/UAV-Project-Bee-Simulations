...
global_odom = np.zeros(6)  # [x, y, z, roll, pitch, yaw]
frame_counter = 0
...
def accumulate_global_odometry(temporal_odom, FEATURE_SHUFFLE_FREQ):
    """
    Accumulates global odometry by adding temporal odometry only at reset points.

    Args:
        temporal_odom (list): [x, y, z, roll, pitch, yaw] for the current window.
        FEATURE_SHUFFLE_FREQ (int): Number of frames between feature resets.

    Returns:
        global_odometry (list): Accumulated global odometry.
    """
    global global_odom, frame_counter

    frame_counter += 1

    if frame_counter % FEATURE_SHUFFLE_FREQ == 0:
        global_odom = [round(g + t, 3) for g, t in zip(global_odom, temporal_odom)]
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        depth_small_avg = depth_small_raw
        # print(depth_small_avg)
        # Inverse depth after averaging
        # depth_small = np.max(depth_small_avg)-depth_small_avg+np.min(depth_small_avg)
        ...
        # odom = accumulate_global_odometry(estimate_odometry(ref_points, matched_points),FEATURE_SHUFFLE_FREQ)
        ...
        # pcd.points = o3d.utility.Vector3dVector(matched_points)
        if first_frame:
