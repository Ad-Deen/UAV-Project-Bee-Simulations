...
def plot_multiple_rays(frame_pos,orientations, length=1.0):
    ...
    for i in range(len(frame_pos)):
        txi , tyi , tzi , rxi , ryi , rzi = frame_pos[i]
        rzi= rzi - 1.5703           
        print(f"for {i} frame the pos {frame_pos[i]}")
...
orientations = [orientations1,orientations2]
pos = [pos1,pos2]

plot_multiple_rays(pos,orientations, length=5.0)
