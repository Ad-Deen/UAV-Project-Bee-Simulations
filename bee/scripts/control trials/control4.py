...
class DroneAscend(Node):
    ...
    def send_throttle_commands(self):
        ...
        target_pos = (0.0, 0, 5.0 , 0.0)  # Example target point (x, y, z , yaw)
        ...
        pitch_offset = 0.0001*self.pitch_trigger       #if pitch offset +ve(pitch_trigger +1) we get Tx -> +ve translation and vice versa
        roll_offset = 0.0001*self.roll_trigger        #if roll offset +ve we get Ty -> +ve translation and vice versa
        yaw_offset = 0.01*self.yaw_trigger              #if yaw offset +v we cw Rz -> +ve rotation and vice versa
